  ...
  persist(
    (set, get) => ({
      ...
      logAccess: (entry) => set((s) => {
        // ID determinístico: prefixo + timestamp ISO sem separadores + counter baseado no tamanho atual
        const id = `log-${new Date().toISOString().replace(/[^0-9]/g, '')}-${s.logs.length + 1}`
        return { logs: [...s.logs, { ...entry, id, timestamp: new Date().toISOString() }] }
      }),
      getLogsForPatient: (patientId) => get().logs.filter((l) => l.patientId === patientId),
    }),
    {
      name: 'imunecare:audit',
      storage: createJSONStorage(() => localStorage),
      merge: (persisted, current) => {
        const p = persisted as Partial<AuditState>
        if (!p?.logs?.length) return current
