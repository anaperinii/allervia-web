...
import { todayStr, maxFutureDateStr } from '@/shared/lib/dates'
          ...
          {props.interval === 'Personalizado' ? (
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <TextInput type="date" max={todayStr()} value={props.startDate} onChange={(e) => props.onStartDateChange(e.target.value)} />
              </div>
              <div>
                <label className="text-[0.6rem] text-(--text-muted) mb-1 block">Data fim</label>
                <TextInput type="date" max={todayStr()} value={props.endDate} onChange={(e) => props.onEndDateChange(e.target.value)} />
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-2">
