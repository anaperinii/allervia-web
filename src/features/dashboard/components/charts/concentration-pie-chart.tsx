...
export function ConcentrationPieChart({
  ...
  // Gera descrição textual para leitores de tela
  const ariaLabel = `Gráfico de pizza: distribuição por concentração. ${data
    .map((d) => `${d.name}: ${d.value}${showCounts ? ' paciente(s)' : '%'}`)
    .join(', ')}.`
