...
function formatDateOrDash(value: string): string {
  if (!value) return '—'
  try {
    return format(new Date(value + 'T12:00'), 'dd/MM/yyyy', { locale: ptBR })
  } catch {
    return '—'
  }
}
...
export function AddImmunotherapyReviewStep({ form }: AddImmunotherapyReviewStepProps) {
  const patientItems = [
    ...
    { label: 'Data de Nascimento', value: formatDateOrDash(form.birthDate) },
  ...
  const immunoItems = [
    ...
    { label: 'Data de Início', value: formatDateOrDash(form.startDate) },
    ...
    <div className="space-y-3.5">
      ...
      <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-lg p-3.5">
        ...
        <p className="text-xs text-amber-800 leading-relaxed">
          Após confirmar, o protocolo será iniciado e a primeira dose será agendada para <span className="font-bold">{formatDateOrDash(form.startDate)}</span>.
        </p>
      </div>
    </div>
  )
