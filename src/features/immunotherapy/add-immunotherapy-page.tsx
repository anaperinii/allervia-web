...
import {
  ...
  type Immunotherapy,
} from '@/features/immunotherapy/stores/immunotherapies-store'
...
import {
  ...
  buildPatientFromImmunotherapy,
} from '@/features/patient/constants/patient-profiles'
...
export function AddImmunotherapyPage() {
  ...
  const canAdd =
    useHasPermission('add_immunotherapy')

  const addImmunotherapy =
    useImmunotherapiesStore(
      (s) => s.addImmunotherapy,
    )
  ...
  const scheduleApplication =
    usePatientStore(
      (s) => s.scheduleApplication,
    )

  const setSelectedPatient =
    usePatientStore(
      (s) => s.setSelectedPatient,
    )
  ...
  const saveImmunotherapy = handleSubmit(
    (data) => {
      const isDuplicateType =
        immunotherapies.some((imm) => {
          if (imm.completed) {
            return false
          }

          if (
            imm.type !== data.type.trim()
          ) {
            return false
          }
      ...
      if (isDuplicateType) {
        toast.danger({
          ...
          title:
            'Tipo de imunoterapia já cadastrado',
      ...
      const modality =
        data.modality as Immunotherapy['modality']
      ...
      const newImm: Immunotherapy = {
        ...
        responsibleDoctor:
          data.responsibleDoctor.trim(),
      }

      addImmunotherapy(newImm)

      const startDatePt =
        isoToPtDate(data.startDate)

      registerPatientProfile(newId, {
        birthDate: isoToPtDate(
          data.birthDate,
        ),
      ...
      toast.success({
        ...
        description: (
          <>
            ...
            foram registrados e a
            próxima dose já está
            agendada.
            ...
            <button
              ...
              className="mt-3 inline-flex cursor-pointer items-center gap-1.5 rounded-md px-2.5 py-1.5 text-left text-xs font-semibold text-teal-600 transition-colors hover:bg-teal-100/50 hover:text-teal-700 active:text-teal-800"
            >
              Acessar prontuário do
              paciente →
            </button>
          </>
        ),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-gray-50/80 p-4">
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
        ...
        >
          ...
          <div className="flex justify-end gap-2 border-t border-(--border-custom) px-5 py-3">
            {step > 1 && (
              <Button
                type="button"
                tone="brand"
