...
const SEED_IMMUNOTHERAPIES: Immunotherapy[] = [
...
export const useImmunotherapiesStore = create<ImmunotherapiesState>()(
  persist(
    (set) => ({
      immunotherapies: SEED_IMMUNOTHERAPIES,
      addImmunotherapy: (imm) => set((state) => ({ immunotherapies: [imm, ...state.immunotherapies] })),
    }),
    {
      name: 'imunecare:immunotherapies',
      // Ao hidratar, preserva os dados persistidos; se estiver vazio (primeiro boot) usa o seed
      merge: (persisted, current) => {
        const p = persisted as Partial<ImmunotherapiesState>
        if (!p?.immunotherapies?.length) return current
        return { ...current, ...p }
      },
    },
  ),
