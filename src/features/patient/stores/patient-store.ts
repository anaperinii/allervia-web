...
// ════════════════════════════════════════════════════════════════════
// Auto-complete: qualquer agendamento anterior ao dia 17/05/2026
// é marcado como concluído e uma próxima aplicação é agendada.
// ════════════════════════════════════════════════════════════════════

const CUTOFF_DATE = new Date(2026, 4, 17) // 17/05/2026 — exclusive boundary

function autoCompletePastApplications(apps: Application[]): Application[] {
  const result: Application[] = [...apps]
  const toAdd: Application[] = []

  // Ordena por data para processar em ordem cronológica
  ...
  for (const app of scheduled) {
    ...
    if (appDate >= CUTOFF_DATE) continue
    ...
    result[idx] = {
      ...
      administratorNote: 'Concluída automaticamente (anterior ao dia 17)',
    }
