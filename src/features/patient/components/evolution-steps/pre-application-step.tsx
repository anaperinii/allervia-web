  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          {/* aria-disabled comunica o estado para tecnologias assistivas */}
          <label
            className={cn(
              'text-xs font-semibold mb-1.5 block',
              sideEffect === 'yes' ? 'text-(--text-muted)' : 'text-(--text-muted)/60',
            )}
            aria-disabled={sideEffect !== 'yes' ? 'true' : undefined}
          >
            Efeitos colaterais relatados {sideEffect === 'yes' && <span className="text-red-400" aria-hidden="true">*</span>}
          </label>
          <TextInput
            placeholder="Insira aqui"
            disabled={sideEffect !== 'yes'}
            invalid={sideEffect === 'yes' && !!errors.reportedEffects}
            className={cn(sideEffect !== 'yes' && 'opacity-60 cursor-not-allowed')}
            aria-required={sideEffect === 'yes' ? 'true' : undefined}
            {...register('reportedEffects')}
          />
          {sideEffect === 'yes' && errors.reportedEffects?.message && (
            <span role="alert" aria-live="polite" className="text-[0.6rem] text-red-500 mt-0.5 block">{errors.reportedEffects.message}</span>
          )}
        </div>
        <div>
          <label
            className={cn(
              'text-xs font-semibold mb-1.5 block',
              medicationNeeded === 'yes' ? 'text-(--text-muted)' : 'text-(--text-muted)/60',
            )}
            aria-disabled={medicationNeeded !== 'yes' ? 'true' : undefined}
          >
            Medicações administradas {medicationNeeded === 'yes' && <span className="text-red-400" aria-hidden="true">*</span>}
          </label>
          <TextInput
            placeholder="Insira aqui"
            disabled={medicationNeeded !== 'yes'}
            invalid={medicationNeeded === 'yes' && !!errors.medications}
            className={cn(medicationNeeded !== 'yes' && 'opacity-60 cursor-not-allowed')}
            aria-required={medicationNeeded === 'yes' ? 'true' : undefined}
            {...register('medications')}
          />
          {medicationNeeded === 'yes' && errors.medications?.message && (
            <span role="alert" aria-live="polite" className="text-[0.6rem] text-red-500 mt-0.5 block">{errors.medications.message}</span>
          )}
        </div>
        <div className="col-span-2">
