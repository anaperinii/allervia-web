...
export function HeroSection() {
  return (
    <section className="min-h-screen pt-30 pb-20 px-[5%] grid grid-cols-1 lg:grid-cols-2 gap-10 items-center relative overflow-hidden">
      <Blob className="-top-40 -right-40 w-125 h-125 bg-linear-to-br from-teal-200/30 to-cyan-200/10" />
      <Blob className="-bottom-40 -left-40 w-112.5 h-112.5 bg-linear-to-br from-cyan-200/20 to-teal-300/20" />
      <Blob className="-bottom-32 -right-20 w-95 h-95 bg-linear-to-br from-teal-200/25 to-cyan-100/15" />
      <Blob className="top-1/3 left-1/2 -translate-x-1/2 w-75 h-75 bg-teal-100/40" />
          ...
          <div className="absolute -bottom-5 -right-5 bg-white rounded-xl shadow-[0_8px_24px_rgba(0,70,40,0.12)] px-3.5 py-2.5 border border-(--border-custom) z-10 w-52">
            ...
            <div className="flex items-center gap-1">
              {CONCENTRATION_STAGES.map((stage) => (
                <div key={stage.conc} className="flex-1 h-1.5 rounded-full" style={{ backgroundColor: stage.active ? stage.color : '#E5E7EB' }} />
              ))}
            </div>
