import { AlertCircle, Calendar, CheckCircle2, Clock, ExternalLink, Phone, Syringe, User } from 'lucide-react'
...
function StatusToggle({ application }: { application: Application }) {
  ...
  return (
    <button
      ...
      onClick={() => setApplicationStatus(application.id, isCompleted ? 'missed' : 'completed')}
      className={cn(
          ...
          <div className="grid grid-cols-2 gap-px bg-(--border-custom) rounded-lg overflow-hidden border border-(--border-custom)">
            ...
            <div className="bg-white px-3.5 py-2.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {(application.status === 'completed' || application.status === 'missed') && (
                  <StatusToggle application={application} />
                )}
                {application.status === 'missed' && application.delayedDays !== undefined && application.delayedDays > 0 && (
                  <span className="text-[0.55rem] text-red-600 bg-red-50 border border-red-200 px-1.5 py-px rounded-full flex items-center gap-0.5">
                    <Clock size={9} />
                    Atrasada {application.delayedDays}d
                  </span>
                )}
              </div>
            </div>
