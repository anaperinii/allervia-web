...
/** Revive o campo timestamp de string ISO para Date após desserialização do JSON */
function reviveNotification(n: Record<string, unknown>): Notification {
  return {
    ...(n as Notification),
    timestamp: n.timestamp instanceof Date ? n.timestamp : new Date(n.timestamp as string),
  }
}
  ...
  persist(
    (set) => ({
      ...
      addNotification: (n) => set((state) => {
        const id = `notif-${new Date().toISOString().replace(/[^0-9]/g, '')}-${state.notifications.length + 1}`
        return { notifications: [{ ...n, id }, ...state.notifications] }
      }),
      markAsRead: (id) => set((state) => ({ notifications: state.notifications.map((n) => n.id === id ? { ...n, read: true } : n) })),
      markAsUnread: (id) => set((state) => ({ notifications: state.notifications.map((n) => n.id === id ? { ...n, read: false } : n) })),
      markAllAsRead: () => set((state) => ({ notifications: state.notifications.map((n) => ({ ...n, read: true })) })),
      markSelectedAsRead: (ids) => set((state) => ({ notifications: state.notifications.map((n) => ids.includes(n.id) ? { ...n, read: true } : n) })),
      markSelectedAsUnread: (ids) => set((state) => ({ notifications: state.notifications.map((n) => ids.includes(n.id) ? { ...n, read: false } : n) })),
      deleteNotification: (id) => set((state) => ({ notifications: state.notifications.filter((n) => n.id !== id) })),
      deleteSelected: (ids) => set((state) => ({ notifications: state.notifications.filter((n) => !ids.includes(n.id)) })),
    }),
    {
      name: 'imunecare:notifications',
      storage: createJSONStorage(() => localStorage),
      // Reconverte timestamps de string ISO de volta para Date ao hidratar
      merge: (persisted, current) => {
        const p = persisted as Partial<NotificationsState>
        if (!p?.notifications?.length) return current
        return {
          ...current,
          notifications: p.notifications.map((n) => reviveNotification(n as unknown as Record<string, unknown>)),
        }
      },
    },
  ),
